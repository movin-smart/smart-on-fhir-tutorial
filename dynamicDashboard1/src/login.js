...
const Login = () => {
    ...
    const fetchRandomImageQuote = async () => {
        try {
            ...
            // const response = await fetch(`http://localhost/refreshemr2/interface/modules/custom_modules/loginscreen/randomImageQuote.php?timestamp=${timestamp}`);
            const response = await fetch(`http://localhost/refreshemr2/sites/randomImageQuote.php?timestamp=${timestamp}`);
            if (!response.ok) {
